class AttachmentsController < ApplicationController
  ...
  def create
    ...
    dir_name = Rails.root.join('attachments',asset_class.downcase.pluralize) 
    FileUtils.mkdir_p dir_name unless File.directory?(dir_name)
     
    File.open(Rails.root.join('attachments',asset_class.downcase.pluralize,attach_file.original_filename), 'wb') do |file|
  ...
  def show
    ...
    send_file Rails.root.join('attachments',attachment.asset_type.downcase.pluralize,attachment.doc_file_name), :type => attachment.doc_content_type
